...
class Order(models.Model):
    ...
    payment_via_paytm = models.ForeignKey(
        "PaymentViaPaytm", on_delete=models.SET_NULL, blank=True, null=True
    )
    coupon = models.ManyToManyField("Coupon", blank=True)
    refund_requested = models.BooleanField(default=False)
    refund_granted = models.BooleanField(default=False)
    order_status = models.CharField(
        choices=order_status_choices, max_length=4, default="ORD"
    )
...
def order_status_handler(sender, instance, created, **kwargs):
    if not created:
        ...
        async_to_sync(channels_layer.group_send)(
            ...
            {"type": "order_status", "payload": json.dumps(data)},
        )
